...
pub fn ktx2_buffer_to_image(
    ...
    let (block_dimensions_x, block_dimensions_y) = texture_format.block_dimensions();
    let (block_width_pixels, block_height_pixels) =
        (block_dimensions_x as usize, block_dimensions_y as usize);
    // 🚧🚧 TODO 🚧🚧: I don't know if that is right 🤷
    let block_bytes = texture_format
        .block_size(None)
        .expect("Get a valid block size") as usize;
